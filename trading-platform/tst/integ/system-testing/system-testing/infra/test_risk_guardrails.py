...
def _make_risk_reg(session_factory, *, cutoff_hour: int = 23, cutoff_minute: int = 59,
                   equity: float = 1_000_000.0, clock=None) -> RiskRegistry:
    return RiskRegistry(
        config=RiskConfig(
            equity=equity,
...
async def test_zero_quantity_rejected(engine, session_factory):
    ...
    risk_reg = _make_risk_reg(
        session_factory, cutoff_hour=15, cutoff_minute=30,

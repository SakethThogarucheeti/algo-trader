...
class _Harness:
    ...
    async def feed(self, candle: CandleEvent):
        ...
        indicators = getattr(self._strategy, "indicators", None)
        if indicators:
            self._ctx.bind_all(indicators, candle.symbol, candle.interval)
...
async def test_rsi_buy_signal_on_oversold_cross() -> None:
    ...
    h = _Harness(strat)
    # Build data: first rising with pullbacks to get RSI below 30, then recover
...
async def test_vwap_buy_signal() -> None:
    ...
    await h.feed(_candle(120.0, ts=session_open, volume=100_000,
    ...
    for i in range(1, 15):
        price = 80.0 - i
        sig = await h.feed(_candle(price, ts=session_open + timedelta(minutes=i * 15),
                                   open=price, high=price + 0.5, low=price - 0.5, volume=1000))
        if sig is not None:
            signals.append(sig)

    # Reversal bar: close higher than previous (momentum turning up) → BUY
    sig = await h.feed(_candle(70.0, ts=session_open + timedelta(minutes=15 * 15),
                               open=65.0, high=70.5, low=64.5, volume=1000))
    if sig is not None:
        signals.append(sig)
...
async def _feed_warmup(h: _Harness, n: int = 10) -> None:
    ...
    for i in range(n):
        await h.feed(_candle(100.0, ts=warmup_start + timedelta(minutes=i * 15),
                             high=105.0, low=95.0))
...
async def test_orb_buy_breakout_above_range() -> None:
    strat = OpeningRangeBreakoutStrategy(
        orb_bars=2, atr_period=_ORB_ATR_PERIOD, interval_minutes=15
    )
    h = _Harness(strat)
    await _feed_warmup(h)
    # 2 range bars establishing OR high=105, OR low=99
    for i in range(2):
        await h.feed(_candle(102.0, ts=_SESSION_OPEN_UTC + timedelta(minutes=i * 15),
                             high=105.0, low=99.0))
...
async def test_orb_only_one_signal_per_session() -> None:
    ...
    sig2 = await h.feed(_candle(114.0, ts=_POST_ORB_UTC + timedelta(minutes=15),
                                high=115.0, low=108.0))
    assert sig2 is None
...
async def test_orb_no_signal_inside_orb_window() -> None:
    strat = OpeningRangeBreakoutStrategy(
        orb_bars=4, atr_period=_ORB_ATR_PERIOD, interval_minutes=15
    )
    h = _Harness(strat)
    # Feed a bar inside the ORB window — no signal regardless of ATR data
...
async def test_orb_no_signal_on_insufficient_atr_data() -> None:
    ...
    h = _Harness(strat)
    # Only 1 candle fed — ATR needs period=3 bars minimum
...
async def test_ema_buy_signal_on_golden_cross() -> None:
    ...
    h = _Harness(strat)
    # Falling prices first to get fast < slow
    for i in range(20):
        await h.feed(_candle(100.0 - i, offset_minutes=i))
    # Rising prices to trigger fast EMA crossing above slow EMA
    signals = []
    for i in range(20):
...
async def test_ema_sell_signal_on_death_cross() -> None:
    strat = EmaCrossoverStrategy(fast=3, slow=7)
    h = _Harness(strat)
    # Rising prices first to get fast > slow
    for i in range(20):
        await h.feed(_candle(100.0 + i, offset_minutes=i))
    # Falling prices to trigger fast EMA crossing below slow EMA
    signals = []
    for i in range(20):

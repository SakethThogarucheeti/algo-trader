...
# ---------------------------------------------------------------------------
# Progress event (published to Redis during a session run)
# ---------------------------------------------------------------------------
...
class TestingSession(ABC):
    ...
    Lifecycle
    ---------
    1. Construct with ``config``, ``bus``, ``results_dir``.
    ...
    def __init__(self, bus: MessageBus, results_dir: Path) -> None:
        self._bus = bus
    ...
    async def _emit_progress(self, event: SessionProgressEvent) -> None:
        """Publish a progress event to the ``testing:progress`` channel."""
        try:
            await self._bus.publish("testing:progress", event)
        except Exception:
            logger.debug("TestingSession: failed to emit progress event", exc_info=True)

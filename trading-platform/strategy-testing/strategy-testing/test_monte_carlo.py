...
Uses fakeredis — no Docker required.
"""
...
async def test_monte_carlo_different_seeds_produce_different_results(tmp_path):
    ...
    sim1 = MonteCarloSimulator(config=config1, trades=trades, bus=bus, results_dir=tmp_path)
    sim2 = MonteCarloSimulator(config=config2, trades=trades, bus=bus, results_dir=tmp_path)

    report1 = await sim1.run()
    report2 = await sim2.run()

    # Compare the raw trial-by-trial return sequences (not summary stats)
    # Two random bootstrap sequences of length 200 will differ in at least one element
    rets1 = report1.return_distribution.to_list()
    rets2 = report2.return_distribution.to_list()
    # At least 1% of trials should differ
...
async def test_same_seed_reproducible(tmp_path):
    ...
    sim_a = MonteCarloSimulator(config=config_a, trades=trades, bus=bus, results_dir=tmp_path)
    sim_b = MonteCarloSimulator(config=config_b, trades=trades, bus=bus, results_dir=tmp_path)

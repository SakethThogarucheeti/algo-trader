...
_RAW = _json.loads((Path(__file__).parents[2] / "strategy_config.json").read_text())
...
async def _run_one(
    oversold: float, overbought: float, atr_mult: float, pg_engine, redis_client, tmp_path
) -> BacktestReport:
    from trading.core.messaging import RedisMessageBus
    schema = f"bt_rsi_{int(oversold)}_{int(overbought)}_{str(atr_mult).replace('.', '_')}"
    combo_bus = RedisMessageBus(redis_client)
    fe_params = (
        dict(_STRAT.feature_engine_params) if _STRAT else {"ema_spans": (9, 21), "atr_period": 14}
    )
    config = BacktestConfig(
        ...
        feature_engine_params=fe_params,
    )
    session = BacktestSession(
        config=config, db_engine=pg_engine, redis=redis_client,
        bus=combo_bus, results_dir=tmp_path, db_schema=schema,
        keep_schema=True,
    )
...
async def test_rsi_grid_search(pg_engine, redis_client, tmp_path):
    ...
    async def _run_and_record(i: int, os_: float, ob: float, atr: float) -> GridResult:
        async with _sem:
            ...
            report = await _run_one(os_, ob, atr, pg_engine, redis_client, tmp_path)
            result = GridResult(

...
async def _run_one(
    ...
    """Run a single combo in its own DB schema + Redis namespace."""
...
async def test_ema_grid_search(pg_engine, redis_client, tmp_path):
    """
    ...
    async def _run_and_record(i: int, fast: int, slow: int, atr_mult: float) -> GridResult:
        async with _sem:
            ...
            report = await _run_one(fast, slow, atr_mult, pg_engine, redis_client, tmp_path)
            result = GridResult(
                fast=fast,
                slow=slow,

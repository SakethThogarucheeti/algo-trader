...
async def _run_one(
    orb_bars: int, atr_mult: float, pg_engine, redis_client, tmp_path
) -> BacktestReport:
...
async def test_orb_grid_search(pg_engine, redis_client, tmp_path):
    ...
    async def _run_and_record(i: int, orb: int, atr: float) -> GridResult:
        async with _sem:
            ...
            report = await _run_one(orb, atr, pg_engine, redis_client, tmp_path)
            result = GridResult(

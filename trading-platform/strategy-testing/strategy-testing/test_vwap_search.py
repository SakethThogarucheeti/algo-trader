...
_RAW = json.loads((Path(__file__).parents[2] / "strategy_config.json").read_text())
...
async def _run_one(
    band: float, atr_mult: float, pg_engine, redis_client, tmp_path
) -> BacktestReport:
...
async def test_vwap_grid_search(pg_engine, redis_client, tmp_path):
    ...
    async def _run_and_record(i: int, band: float, atr: float) -> GridResult:
        async with _sem:
            ...
            report = await _run_one(band, atr, pg_engine, redis_client, tmp_path)
            result = GridResult(
